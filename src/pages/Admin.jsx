...
// Default cards so UI never empties
...
export default function Admin() {
  ...
  async function loadAll() {
    try {
      ...
      // --- PRODUCTS ---
      if (pRes.ok) {
        // clone so we never re-read a body by accident
    ...
    try {
      ...
      const res = await fetch("/api/admin/products", {
        ...
        body: JSON.stringify({
          ...
          name: p.name,
          price_cents: cents,
          // Mirror string field for strict validators
          price: String(cents),
          active: !!p.active,
        }),
      });
      ...
      setProducts(prev => {
        ...
        // ensure we keep our local cents if backend returned nothing
        if (merged.price_cents == null) next[idx].price_cents = cents;
        return next;
      });
    } catch (e) {
  ...
  async function saveJackpot(e) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/jackpot", {
        ...
        credentials: "include",
        headers: { "Content-Type": "application/json", ...authHeaders() },
        // Send numeric and string to satisfy schema expecting a string pattern
